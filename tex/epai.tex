...
\def\dofigure#1#2#3#4{\par
 ...
 \ifcase\pdfoutput
  \special{pdf: dest (num \number\count255) [@thispage /FitBH @ypos]}\else
  \pdfdest num \number\count255 fitbh\fi
...
\def\ndxone#1#2#3{\let\oldp@ge=\newp@ge\def\newp@ge{#3}%
 \ifx\newk@y\oldk@y
  \ifx\newp@ge\oldp@ge \else, \goto{#3}{#1}\fi
 \else \par\noindent\hang{\sc\newk@y},\space\space
  \goto{#3}{#1}\fi\ignorespaces}
...
\def\toclinezero#1#2#3#4{\bigskip\null
 \def\3{#3}\ifx\empty\3\def\1{#1}\else\def\1{#3. #1}\fi
 \centerline{\lmzero\goto{\1}{#2}}}
\def\toclineone#1#2#3#4{\medbreak
 \def\3{#3}\ifx\empty\3\def\1{#1\tocdotfill}\else\def\1{#3\quad#1\hfill}\fi
 \line{{\bf\1\space}\goto{#4}{#2}}}
...
Responde, entre otras, a cuestiones tales como:
 ...
 ¿Qué es un vaso?\par
 ¿Qué es la vida?\par
 ¿Qué soy yo?\kern4.4cm\vbox to 0pt{\vss
  \hbox{%
   \ifcase\pdfoutput
    \special{pdf: sc [\cmykBlack]}%
    \special{pdf: content
     q 0 0 0 0 k 0 0 0 1 K -5 -5 m 110 -5 l 110 92 l -5 92 l -5 -5 l b Q}%
   \else
    \pdfliteral{\cmykBlack\space k}%
