...
\def\pdfCinnamon{\pdfliteral{0 0.49 1 0.52 k 0 0.49 1 0.52 K}}
...
%\def\nosection{\let\section\empty}
\def\nosection{\def\section{Z}}

%\def\section{\ifnum\s@one>0
\def\section{\ifnum\s@one=0 0\else
...
\def\titlezero#1 \par{\newpage\vglue\prologskip
 \centertitle{\pdflabel\lmzero #1}\parrafalse\nobreak\bigskip}
\def\titleone#1 \par{\ifparra\newpage\else\bigskip\fi
 \noindent\pdflabel{\lmone\section\quad #1}\toc1{#1}%
...
\def\titletwo#1 \par{\ifparra \par\penalty-200\bigskip \else\smallskip\fi
 \noindent\pdflabel{\lmtwo\section\quad #1}\toc2{#1}%
 \iflabeled\setbox0=\hbox{\thelabel}\ifdim\wd0=0pt\label{#1}%
  \else\label{\thelabel}\let\thelabel\null\fi\labeledfalse\fi
 \par\parrafalse\nobreak\smallskip}
\def\titlethree#1 \par{\ifparra \par\penalty-100\medskip \fi
 \noindent\pdflabel{\bf\section\quad #1}\toc3{#1}%
 \iflabeled\setbox0=\hbox{\thelabel}\ifdim\wd0=0pt\label{#1}%
  \else\label{\thelabel}\let\thelabel\null\fi\labeledfalse\fi
 \par\parrafalse\nobreak\smallskip}
\def\titlefour#1 \par{\ifparra\smallbreak\fi\noindent\pdflabel{\bf#1}\toc4{#1}%
 \iflabeled\setbox0=\hbox{\thelabel}\ifdim\wd0=0pt\label{#1}%
...
\def\newpart#1{\newpage \null
 \headline={\lmviiirm\ifodd\pageno \hfil\sectionname \else
  \uppercase{#1}\hfil\fi}\newoddpage{\let\section\empty\pdflabel\toc0{#1}}}
\def\newPart#1#2{\newpage \null
 \headline={\lmviiirm\ifodd\pageno \hfil\sectionname \else
  \uppercase{#2}\hfil\fi}\newoddpage{\def\section{#1}\pdflabel\toc0{#2}}}
\def\sectionname{}

% PROLOG
\def\prolog#1 (#2){\ifparra\newpage
 \hrule height0pt\vskip\prologskip\else\bigskip\bigskip\fi
 \centerline{\pdflabel\toc1{#1}\vbox{\lmone \openup3pt
...
\def\label#1{\leavevmode\pdflabel\lbl{#1}{}\ignorespaces}
\def\person#1{\leavevmode\pdflabel\ndx{#1}1\penalty\@M\hskip\z@skip{\sc #1}}
\def\index#1{\leavevmode\pdflabel\ndx{#1}0\penalty\@M\hskip\z@skip\ignorespaces}
\def\tocfig#1{\leavevmode\pdflabel\toc9{#1}\global\advance\d@stno1\ignorespaces}
